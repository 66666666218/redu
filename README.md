...
多租户热点监控 + 管理后台。自动采集微博热点、闲鱼虚拟商品、抖音热点宝、百度热搜、
微信公众号文章(免费,基于微信读书),跨轮判涨、苗头 Agent 预测(增速/共振/生命周期),
网盘资源自动转存换链,AI 改写+解读(DeepSeek),飞书六群分流推送,后台可管理用户/角色/权限/日志/报表。

## 公众号网盘资源监控(零成本运行)

- **自动监听** 21+ 对标号的新发文(微信读书 cover,60 分钟轮询)
- **自动转存** 盘链文章的资源到你的夸克网盘,推送带你的分享链接
- **阅读量** 微信读书站内精确值(免费)+ dajiala 全网六指标(¥0.06/篇,可选)
- **苗头 Agent** 五信号加权评分 + 生命周期追踪(苗头→上升→爆发→回落)
- **关键词文章监控** 全网搜索不限对标号(每 4h 轮转)
