...
- **资源去重键 `resource_key(title)`**(2026-09-22):去空白/小写之外再剥掉"【自动发货】/秒发/拍下即发"这类**发货话术**与符号(剥完不足 4 字则退回不剥,防两字键错并),三处共用——① `collect_hot` 单轮内标题归并;② `run_xianyu` 跨轮入库(24h 内同键视为同资源,不再因换 ID 重发而重复插条);③ `dedupe_resources` 读侧折叠(大盘/闲鱼页多取 4 倍再按键去重,消窗口外重上架的历史重复行)。
...
- **定向查询**:`douhot_client.hot_word_keyword(keyword)` → 查接口;`douhot.fetch_keyword_heat`
  ...
  搜出的**每个相关主题各记一条快照**(`entry_title`=该主题标题,单次 ≤ `DOUHOT_WATCH_ENTRY_CAP`
  默认 **100** 条——话题榜实测可取满 100,搜索/视频受服务端上限约 50),从而**逐条追踪趋势**;
  `subscribe` 无 keyword,仍在榜单内查找。
...
- **展示**:仪表盘"关键词监控 · 智能体"卡片——当前分、环比、预测下一轮、迷你趋势线、
  ...
  **2026-09-05** 摘要不再笼统说"该词上升",而是**点名具体主题**(如 `追踪3主题 · 上升:早春晴朗·酷酷+80%`、
  或 `新增:XX`)——因为榜单搜索类的关键词内含多条主题、趋势各异,概括会误导。
  卡片标题徽章额外返回 `trend_overview`(如 `3主题 · 升1 · 新1`),比单一 `trend_label` 更明确。
...
1. **每日热点日报** `run_feishu_daily()`:按 `FEISHU_DAILY_CRON`(默认每天 08:00)推送四板块
   ...
   底部【关键词关注 · 智能体】给出各关注词的**趋势/环比/预测/置信度**。**2026-09-05 增强**:
   榜单搜索类关注(话题/搜索/视频,逐条记录)会列出当前监测到的 **Top 相关主题**(默认
...
2. **实时提醒** `run_feishu_realtime(section, user_id)`:每次采集成功后调用,只把**当日新增**、
   **排名跳升 ≥ `FEISHU_HOT_RANK_JUMP` 名**、或**分值环比 ≥ `FEISHU_HOT_RATIO`** 的话题立即推群,
   ...
   让"飙升"不只是数字、还带智能体预测。**2026-09-05 新增 `run_feishu_keyword_realtime`**:
   抖音**话题词(榜单搜索类)**采集后,若检测到**新进 / 上升期 / 预测爆发**的主题,即推一条
   `📌 话题词监控 · 关键词`(只列个别变化主题,`kw_realtime` 冷却去重,不刷屏),与每日日报(08:00)
   全量 Top100 互补——让你及时看到话题词波动而**不必等每日 08:00**。挂在手动采集接口与调度器
   `collect_tick` 的抖音分支(与 `run_feishu_keyword_alerts` 并列)。
...
### 5.14 早期苗头 Agent `services/early_agent.py`(全板块自主预测闭环)

围绕业务目标"第一时间捕捉爆点做网盘推广、避开衰败期"的五步闭环 Agent:
感知(各板块热度/想要/指数序列)→ 联想(归一化+包含匹配跨板块共振)→ 评估(多信号加权 0~100 分:
增速≥50%+30/≥100%+40、加速上涨+20、新上榜+25、连续≥3轮+15、量级≥200+15、共振+30)
→ 决策(生命周期状态机 `agent_stages` 表持久化:观察→苗头(≥55)→上升(≥70)→爆发(≥85)→回落,
**仅阶段跃迁时推送**——苗头首推、升级推送、跌入回落止损提醒,同阶段绝不重复)→ 行动(column_set 红卡,
带阶段徽标/信号分解/网盘推广行动建议/跟踪时长;共振推总群、单板块推专属群)。

- 调度:独立 tick 每 30 分钟(`agent_tick_all_users`)+ 每次采集成功后即时评分(weibo/xianyu/douhot/baidu)。
- 配置:`AGENT_ENABLED`(默认开)/`AGENT_SCORE_THRESHOLD=55`/`AGENT_COOLDOWN_HOURS=12`(跃迁之外不可用,
  同阶段由状态机去重)/`FOCUS_MIN_LEN`(共振匹配最短词长)。
- 各板块信号源:微博热度序列、闲鱼 XianyuDaily 想要数序列(snap_date 字符串日期,`_to_dt` 容错)、
  抖音 score 序列、百度热度序列;公众号板块的早期信号由 wechat_monitor 的基线/增长/资源共振承担。
- **板块差异化信号(v2.1)**
- **自学习(v2.2)**:`agent_learning.py` 每日 06:00 回测两天前的苗头(爆发=最新热度≥推送时×1.5),按信号标签累计命中率;样本≥10 时自适应调整权重(±30% 锁定),持久化 `system_config(agent_weights)`。
- **网盘 Cookie 保活巡检**:`pan_cookie_keepalive_tick` 每日 07:00(作业名 `pan_cookie_keepalive`)逐用户探
  夸克(轻量列目录,滚动延长 __puus)+ 百度(`loginStatus`),失效即时告警走 wechat 板块群(转存暂停但监听不受影响)。
  同一份 Cookie 只探一次、只告警一次;禁用用户不探;网络/风控类异常只记日志不报"失效"(否则天天误报)。
- **自学习权重信号**:微博/百度额外接入**排名速度**(近24h排名前移≥3名 +15)——排名比热度
  对起势更敏感;加速度(增速比再升≥20个百分点 +20)是全板块最早的起势标志。
- **实时性说明**:微信读书 App 端是推送式秒级同步(书架红点);我们走轮询(cover 接口),
  且为压低风控改成**定点作业**(每天 4 次),`schedule_service.WECHAT_LISTEN_HOURS` 是单一事实源
  (调度 CronTrigger、界面"下次预计"、健康页停滞基准都读它)。当前取 **4:00/8:00/14:00/20:00**
  (2026-09-22 用户决策,原 2:00/8:00/14:00/18:00),最坏延迟 = 夜间空档 8h、平均约 4h。
  用户的 `interval_minutes` 对公众号已不参与调度(`claim_schedule(force=True)` 无视间隔),
  保留字段只为兼容与界面展示。
...
- **架构决策(保持单文件,已实验验证)**:曾实际执行拆分为包(_common/_benchmarks/_listen/_sync/
  ...
  拆分后目标移到子模块,十几个测试同时破裂;③ 文件内 7 个域共享大量符号,拆分引入的导入管理
  复杂度超过可读性收益。**替代方案**:文件内已按 `# --- 域 ---` 注释分区,导航靠此;未来若拆分,
  应先消除跨域私有引用(把共享工具下沉 _common)再做物理移动。
- 已知边界:序列日级粒度的板块(闲鱼)苗头窗口天然以天计;公众号文章级信号见 §5.8b。
