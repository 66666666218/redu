...
- **架构决策(拆分暂缓)**:`wechat_monitor.py`(1180+ 行)按域拆包(listen/sync/traffic/candidates/
  benchmarks)——**暂缓执行**。原因:该文件被多工作流并行演进(候选发现/Cookie续期),结构性移动
  会产生严重合并冲突;待并行改动稳定收敛后按上述域拆分,`__init__` 全量 re-export 保持兼容。
